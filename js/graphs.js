...
function renderCharts() {
	try {
		...
		function drawScatterMatrix(data) {
			...
			if (!container) return;
			// Remove ALL SVGs in container (including previous scatter matrix SVGs)
...
function drawLine(data, startYear, endYear) {
	...
	const bottomPad = 100;
	// Don't set explicit width/height - let CSS handle it to avoid expanding width bug
...
d3.dsv(';', csvPath).then(raw => {
	...
	// Wait for layout to be painted before measuring dimensions
